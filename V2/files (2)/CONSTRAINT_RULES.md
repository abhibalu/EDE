...
Assembly is deterministic code (not LLM). These findings are produced during fragment merging:

| Rule ID | Node | What it checks |
|---------|------|---------------|
| ASSEMBLE-dedup | 1 | Same event (name + file) found by multiple sub-agents — keeps first |
| ASSEMBLE-unresolved-predecessor | 1 | Predecessor event name not found in any fragment |
| ASSEMBLE-unresolved-successor | 1 | Successor event name not found in any fragment |
| ASSEMBLE-symmetry-fix | 1 | Auto-adds missing symmetric link (INFO, not error) |
| ASSEMBLE-unresolved-event | 2 | Transition references event name not in Node 1 catalogue |
