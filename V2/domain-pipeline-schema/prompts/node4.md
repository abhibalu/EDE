...
- `docs/pipeline/output/00-recon.json`
- `docs/pipeline/output/01-events.json`
- `docs/pipeline/output/02-statecharts.json`
- `docs/pipeline/output/03-goals.json`
...
## Step 1: Produce the JSON Output
...
Scan ALL requirements and their resolved obstacles back to gaps and code locations. For each unique source file referenced anywhere in the pipeline:
1. Determine its category from the directory structure
2. List which requirements touch it (directly or through the gaps/obstacles they resolve)
3. Do NOT guess — only include files that are explicitly referenced
...
Write this to: `docs/pipeline/output/domain-spec.md` and `docs/domain-spec.md`
