...
Create a dated run directory before writing output:
```
mkdir -p docs/pipeline/runs/YYYY-MM-DD
```
where YYYY-MM-DD is today's date.

Write output to: `docs/pipeline/runs/{RUN_DATE}/00-recon.json`
All subsequent nodes will use this same `{RUN_DATE}` directory.
